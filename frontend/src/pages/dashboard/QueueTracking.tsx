...
import { 
  ...
  FaTimesCircle, FaExclamationTriangle, FaSearch, FaLayerGroup, FaCut, FaArrowLeft
} from 'react-icons/fa';
import { queueApi, QueueEntry } from '../../services/serviceService';

const QueueTrackingPage: React.FC = () => {
  const { referenceNumber: urlRef } = useParams<{ referenceNumber?: string }>();
  const [referenceNumber, setReferenceNumber] = useState(urlRef || '');
  ...
  const getQueueStatusDetails = (status?: string, approvalStatus?: string) => {
    // If booking is pending review and not approved yet
    if (approvalStatus === 'waiting') {
      return { 
        text: 'Waiting for Approval', 
        color: 'bg-yellow-500/10 text-yellow-400 border border-yellow-500/30',
        desc: 'Our staff will review and approve your booking shortly.'
      };
    }
    if (approvalStatus === 'rejected') {
      return { 
        ...
        desc: 'This booking has been rejected by our team.'
      };
    }
    ...
    switch (status) {
      ...
      case 'queued': return { 
        ...
        desc: 'You are currently in the queue. Please monitor your position below.'
      };
      case 'serving': return { 
        text: 'Currently Serving', 
        color: 'bg-amber-500/10 text-amber-400 border border-amber-500/30',
        desc: 'Your turn is up! Please proceed to the service station immediately.'
      };
      case 'completed': return { 
        text: 'Service Completed', 
        color: 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/30',
        desc: 'Thank you for choosing us! We hope to see you again soon.'
      };
      default: return { 
        text: 'Active Booking', 
        color: 'bg-slate-800 text-slate-300 border border-slate-700/50',
        desc: 'Your booking details are logged.' 
      };
    }
  };

  const statusInfo = getQueueStatusDetails(queueData?.queue_status, queueData?.approval_status);

  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col items-center justify-center p-6 md:p-8 font-sans">
      <div className="w-full max-w-lg space-y-6">
        ...
        {/* Input Reference Panel (only when not direct url link) */}
        {!urlRef && (
          <motion.div 
          ...
          >
            <h2 className="text-xl font-extrabold text-slate-100">Track Customer Queue</h2>
            <p className="text-xs text-slate-400 mt-1">
              Enter your booking reference number below to monitor wait times and positions.
            </p>
        ...
        {loading && urlRef && (
          <div className="flex flex-col items-center justify-center py-20 bg-slate-900/40 backdrop-blur-sm rounded-2xl border border-slate-850">
            ...
            <p className="text-slate-400 text-sm font-medium">Tracking booking reference status...</p>
          </div>
        )}
        ...
        {error && !loading && (
          ...
          >
            ...
            <div>
              <h4 className="font-bold text-red-400 text-sm">Failed to Locate</h4>
              <p className="text-slate-400 text-xs mt-1 leading-relaxed">{error}</p>
              {!urlRef && (
                <p className="text-slate-500 text-2xs mt-2">
                  Please verify the spelling and try again. E.g. BRB-2026-0001
                </p>
              )}
            </div>
          </motion.div>
        ...
        {/* Active Queue Details Dashboard */}
        {!loading && queueData && (
          <motion.div
            initial={{ opacity: 0, y: 15 }}
            animate={{ opacity: 1, y: 0 }}
            className="bg-slate-900/60 backdrop-blur-md border border-slate-850 rounded-2xl p-6 space-y-6 shadow-2xl relative overflow-hidden"
          >
            {/* Status Header Badge Banner */}
            ...
            <div className="bg-slate-950/40 rounded-xl p-4 border border-slate-900/80 space-y-3">
              ...
              <div className="flex justify-between items-center text-sm border-b border-slate-900 pb-2">
                ...
                <span className="font-extrabold text-slate-200">{queueData.barber_name || 'Awaiting Selection'}</span>
              </div>
              <div className="flex justify-between items-center text-sm border-b border-slate-900 pb-2">
                <span className="text-slate-500 font-semibold">Client Profile</span>
                <span className="font-extrabold text-slate-200">{queueData.customer_name}</span>
              </div>
              <div className="flex justify-between items-center text-sm">
                <span className="text-slate-500 font-semibold">Scheduled Date/Slot</span>
            ...
            {/* Queue Board Widgets */}
            {queueData.approval_status === 'approved' && queueData.queue_status !== 'completed' && (
              <div className="p-5 bg-blue-600/5 border border-blue-500/20 rounded-2xl space-y-4">
                <div className="flex items-center justify-between">
                  <div>
                    <span className="text-2xs font-semibold text-slate-400 uppercase tracking-wider block">Position in Queue</span>
                  ...
                  <div className="text-right">
                    <span className="text-2xs font-semibold text-slate-400 uppercase tracking-wider block">Estimated Wait</span>
                ...
                <div className="space-y-1">
                  <div className="bg-slate-800 rounded-full h-2">
                    <div 
                      ...
                      style={{ width: Math.max(5, 100 - (queueData.queue_position * 15)) + '%' }}
                    ></div>
                  </div>
                  <div className="flex justify-between text-3xs text-slate-500 font-bold uppercase tracking-wider">
                    <span>In Queue</span>
            ...
            {/* Stage Status Description Message */}
            ...
            {/* Auto refresh notice */}
            <p className="text-3xs text-slate-650 text-center uppercase tracking-widest font-semibold">
              Live updates active • Refreshes in background
            </p>
          </motion.div>
        )}
      </div>
