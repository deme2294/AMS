...
console.log('Testing manage complaints with proper authentication...');
        ...
        try {
            ...
            if (loginResult.success) {
                ...
                if (token) {
                    ...
                    // Step 2: Test manage complaints with authentication
                    testManageComplaintsWithAuth(token);
                } else {
...
function testManageComplaintsWithAuth(token) {
    console.log('\n--- Testing Manage Complaints API with Authentication ---');
        ...
        manageRes.on('end', () => {
            ...
            try {
                ...
                if (result.success && result.data) {
                    console.log('✅ Manage complaints API working with authentication!');
                    console.log('✅ Can fetch complaints from database!');
                    
                    if (result.data.length > 0) {
                        console.log('Sample complaint:', {
                            id: result.data[0].complaint_id,
                            title: result.data[0].title,
                            status: result.data[0].status,
                            reference: result.data[0].reference_number
                        });
                    }
